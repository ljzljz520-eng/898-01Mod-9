...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-semibold text-neutral-800">创建楼栋</h1>
    <p class="text-sm text-gray-500 mt-1">添加新的楼栋信息，便于居民认证和管理</p>
</div>
...
<div class="card max-w-2xl">
    <form method="POST" action="{{ route('buildings.store') }}">
        ...
        <div class="mb-4">
            ...
            <input type="number" id="total_units" name="total_units" value="{{ old('total_units') }}" min="0"
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="4"
                      ...
                      placeholder="请输入楼栋描述信息（可选）">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('buildings.index') }}" class="btn-secondary">取消</a>
            <button type="submit" class="btn-primary">创建楼栋</button>
        </div>
    </form>
</div>
